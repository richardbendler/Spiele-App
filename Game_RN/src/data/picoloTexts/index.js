...
const { getCategoryColor, normalizeCategoryKey } = require('../../utils/categoryColors');
...
const buildPrompt = (() => {
  ...
  return (poolKey, template) => {
    ...
    const manualApproval =
      typeof template.manual_approval === 'number'
        ? template.manual_approval
        : typeof template.manualApproval === 'number'
          ? template.manualApproval
          : typeof template.manuellFreigegeben === 'number'
            ? template.manuellFreigegeben
            : 0;
    const customPayload =
    ...
    return {
      ...
      manual_approval: manualApproval ? 1 : 0,
    };
  };
})();
...
const toPrompt = (entry) => {
  ...
  const normalizedKey = normalizeCategoryKey(poolBase.key);
  const poolKey =
    (typeof normalizedKey === 'string' && normalizedKey.length > 0
      ? normalizedKey
      : typeof poolBase.key === 'string' && poolBase.key.length > 0
        ? poolBase.key
        : 'mixed');
  const poolColor = getCategoryColor(poolKey) || entry.pool_color || '#2F4F4F';
  ...
  const manualApproval = typeof entry.manual_approval === 'number' ? entry.manual_approval : 0;
